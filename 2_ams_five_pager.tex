...
transaction costs, and how consultants fit into that picture. ``As
...
Since managers are only boundedly rational, the assumption is that there
is a high demand for advice and (IT) solutions that improve coordination
within and between firms. Services in which consultants are particularly
well-versed. The question to ask here is: is the transaction cost for
working with external consultants lower than for working with internal
consultants when it comes to knowledge production?

\citet[37-44]{canback1998} argues it does. According to his
interpretation of transaction cost economics, the choice depends on
three factors.
...
\tightlist
\item
  The degree of asset specificity: do physical, human or site assets
  have a specific usage and can they not be put to use for another
  purpose? Higher asset specificity leads to internalization, since no
  vendor is willing to tailor his product or service to one client,
  facing downward price pressure, since the latter acts as a monopsonist
  \citep[ 218-228]{robinson1969}.
\item
  Uncertainty: business cycle volatility and technical uncertainty lead
  to more coordination and bureaucratic transactions since it is hard
  and expensive to work with contracts that cover a wide range of
  contexts and possible outcomes. Consequently, uncertainty leads to
  internalization.
\item
  The frequency of transactions: if transactions are frequent, there
  will be a higher bureaucratic cost involved, once again leading to a
  tendency to internalize the good or service.
\end{enumerate}
...
human low asset specificity, since the solution or advice can be
reproduced at many organizations. While it is true that consultancy
assignments involve relatively few transactions (often a single one),
Canback claims that transaction frequency and uncertainty are less of an
influence, obscuring the fact that consultants rather thrive in a
context with high complexity and uncertainty.

Argument via Williamson \& Godfrey: it's about behavioral uncertainty.
...
features that signal status, quality and reliability. In consulting this
translates into degrees, certificates, using the adequate buzzwords,
wearing a suit and driving a quality car.
